    ...
    <div class="">
        ...
        <div class="widget-content" >
          <table class="table table-striped table-bordered">
            <thead>
              <tr>
                <th width="80%"><i class="icon-list"></i> RETURNING STUDENTS</th>
              </tr>
            </thead>
            <tbody>
              <tr >
         ...
         <form name="enrollment" action="reg_enroll-action.php" method="post">	
          ...
          <table cellpadding="0" cellspacing="0" border="0" class=" " id="tab<?php echo $lvl;?>" width="100%">
              ...
              <tbody>
                 ...
                 <?php 
					...
					$query_schyr1=mysql_query("SELECT sy_id FROM school_year_t WHERE SY_Status=1");

								$row_schyr1=mysql_fetch_assoc($query_schyr1);
								$schyr_id=$row_schyr1['sy_id'];
					...
					$query_reg=mysql_query("SELECT * FROM student_registration_t WHERE school_yr=$prev AND year_level=$lvl") or die(mysql_error());
							while($row_reg=mysql_fetch_assoc($query_reg)){
								$id=$row_reg['student_id'];
					$query_student=mysql_query("SELECT * FROM student_registration_t WHERE student_id='$id' AND school_yr=$schyr_id");
					$student=mysql_num_rows($query_student);		
			
								
					$query_fg=mysql_query("SELECT * FROM final_grade_t WHERE student_id='$id' AND sy_id=$prev");
							while($row_fg=mysql_fetch_assoc($query_fg)){
								$fg=$row_fg['final_grade'];
								
					$query_name=mysql_query("SELECT * FROM student_t WHERE student_id='$id'");
							...
							if($student==0){
								...
								if(($fg>75) && ($lvl==6))	{
									 $query_up=mysql_query("UPDATE student_t SET student_type='Alumni' WHERE student_id='$id'");
								} else if (($fg>75) && ($lvl!=6)){
									?>
                        ...
                        <tr class="del<?php echo $id ?>">
                            ...
                            <td><?php echo strtoupper("".$row_name['l_name']." , ".$row_name['f_name'].""); ?>
                            	<?php echo ucfirst("".$row_name['m_name']."") ?></td>
			...
			<br />
					<input type="hidden" name="schyr_id" value="<?php echo $schyr_id;?>">   
					<p align="center"><input type="submit" name="Enroll" value="Enroll" class="btn btn-success btn-large"></p>
