...
@session_start();
		...
		$qry61=mysql_query("SELECT count(isbn) as max from cat_books_t WHERE isbn='$isbn'");
		...
		if($countrm=='0')
		{
			...
			if($countbook=='0')
			{
				...
				for($s=0; $s<$copies; $s++){
				$max1=$max1+000001;
				mysql_query("INSERT INTO `cat_copies_t`VALUES ('$max1','$callno','On shelf','$account_no','$supplytype')");}
	...
	<?php }
	else if($countrm != '0' && $countisbn == '0')
	{echo"<p class='error-box'> Call no. already exists</p>";}	
		else{$account_no=$_SESSION['account_no'];
	...
	<?php	}
		...
		?>
			...
			<td width="40"  colspan="8" style="background-color:#969696;font-size:23px; font-family:Georgia, 'Times New Roman', Times, serif"><strong>BOOK INFORMATION</strong></td>
                    ...
                    <input style="width:auto" type="checkbox" name="ill"/>&nbsp;with illustration
	 				</div></div>
					</td><td>
					<div class="control-group ">
