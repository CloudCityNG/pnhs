	...
	<!-- MAIN -->
          ...
          <div id="result"></div>
						<div id="page-title">
							...
							<span class="subtitle"><br>“When I got my library card, that was when my life began.” 
</br>
― Rita Mae Brown
</span>
						</div>
			...
			<div class="three-fourth">
      		...
      		<div class="widget stacked">
				...
				require "db.php";
					...
					$query = mysql_query("SELECT * FROM appraisal_t WHERE return_time='0' and access_no='$access_no' and account_no='$account_no'");  
					while($q=mysql_fetch_array($query)){
					$account = $q[account_no]; 
					$access = $q[access_no];
					$rm = mysql_fetch_array(mysql_query("SELECT * FROM cat_copies_t WHERE access_no='$access_no'"));  
					$call_no=$rm['call_no'];
					$se = mysql_fetch_array(mysql_query("SELECT * FROM cat_reading_material_t WHERE call_no='$call_no'"));
					$section=$se['section_no'];
					$title=$se['title'];
					...
					$final=$in_time+$in_date;//time n dapat ibalik
					$mark="On Shelf";
					//echo"final=$final";
					...
					//	echo"unit=$seconds";
						...
						$amount11=($seconds/$unit2);
						$amount111=$amount11*$amount;
						//echo"$amount111==$seconds==$unit2==$amount11 <br>$time2<br>$amount";
					
						//echo"unit$time2<br>$unit1<br>$amount1";
						
						$amount2=number_format($amount111, 2, '.', ',');
					...
					if(($access_no==NULL)&&($_no == NULL))
					{
					echo"<p class='error-box'>Book ID and Library ID are both EMPTY.</p>";
					}
					else if($_no== NULL)
					{
					echo"<p class='error-box'>Library ID is EMPTY.</p>";
					}
					else if($access_no==NULL)
					{
					echo"<p class='error-box'>Book ID is EMPTY.</p>";
					}
					else if(($account != $account_no)&&($access!=$access_no)) 
					{
					echo"<p class='error-box'>Library ID and Book ID do not match.</p>";
					}
					else if($account != $account_no) 
					{
					echo"<p class='error-box'>Library ID number do not match.</p>";
					}
					else if ($access != $access_no)
					{
					echo"<p class='error-box'>BOOK ID number do not match.</p>";
					}
					
					else if(($account_no==$account) &&($access_no==$access))
					{				
						...
						{
							$time2=$time1/60;
							echo"<p class='success-box'><b>Successfully Returned!
							<br>Title:&nbsp;&nbsp;&nbsp;$title
							<br>Time Allotment:&nbsp;&nbsp;&nbsp;$time3 $unit(s)
							<br>Remaining Time:$day1 Day(s) $hourrs1 Hour(s) $minuter1 Minute(s) $seconder1 Second(s) 
							<br>Returned by: <br>
							Library ID:&nbsp;&nbsp;&nbsp;$_no<br>
							Name:&nbsp;&nbsp;&nbsp; $name1 $name2 $name3
							<a style='color:#FFFFFF' id='contactForm' title='press to pay' href='save_book_return.php?access_no=$access_no&&account_no=$account_no'>";
							?>
							<br><input type="submit" value="OK" name="submit2" id="submit" required /></p>
					</a> 
						</p>
							<?php 			
						}
						...
						{				
							echo"<p class='error-box'><b>You committed OVERDUE!
							<br>Title:&nbsp;&nbsp;&nbsp;$title
							<br>Time Allotment:&nbsp;&nbsp;&nbsp;$time3 $unit(s)
							<br>Overdue time:$day Day(s) $hourrs Hour(s) $minuter minute(s) $seconder second(s) 
							<br>Unit Cost/$unit_cost:&nbsp;&nbsp;&nbsp;Php $amount
							<br>Penalty Amount:&nbsp;&nbsp;&nbsp;Php $amount2
							<br>Returned by: <br>
							Library ID:&nbsp;&nbsp;&nbsp;$_no
							<br>Name:&nbsp;&nbsp;&nbsp; $name1 $name2 $name3
							<br>";?>
							<a style="color:#FFFFFF" id="contactForm" title="press to pay" href='paid.php?access_no=<?php echo"$access_no";?>&&account_no=<?php echo"$account_no";?>&&amount=<?php echo"$amount3"; ?> '><input type='submit' value='PAID' name='submit2' id='submit' required /></a> 
									<?php 
									echo"<a id='contactForm' style='color:#FFFFFF' href='cancel.php?account_no=$account_no&&access_no=$access_no&&amount=$amount3'> <input type='submit' value='CANCEL' name='$amount111' id='submit' required /></a>
									</b></p>";
												mysql_query("Commit");
...
?><form id="contactForm" action="#" method="post">
								<fieldset>
									<div>
										<label>Book ID</label>
										<input style="height:inherit" name="access_no"  id="name" type="text" class="form-poshytip" title="Enter your book ID" />
									</div>
									<div>
										<label>Library ID</label>
										<input style="height:inherit" name="account_no"  id="email" type="text" class="form-poshytip" title="Enter your Library ID" />
									</div>
									<div>
										
									<p><input type="submit" value="RETURN" name="submit" id="submit" /></p>
								</fieldset>
								</form>
  	<!-- ENDS form -->
							</div>
					...
					<div class="one-fourth last">
						...
						<ul id="sidebar">
							...
							<li class="cat-item"><a  href="book_borrow.php" title="Click to Borrow book" >Borrow Books</a></li>
									<li class="cat-item"><a href="book_return.php" title="Click to Return book" style="background:#f1f1f1">Return Books</a></li>
									<li class="cat-item"><a href="book_lost.php" title="Click to Report Book Lost">Report Book Lost</a></li>
									<li class="cat-item"><a  class="submit" onClick="window.open('bago.php?','','toolbar=no, location=no, directories=no, status=no, menubar=no, scrollbars=no, resizable=no, copyhistory=no, 1100, height=650,position=center');"title="Click to add book" ">Add Books</a></li>
                             		<li class="cat-item"><a  class="submit" onClick="window.open('periodical_add.php?','','toolbar=no, location=no, directories=no, status=no, menubar=no, scrollbars=no, resizable=no, copyhistory=no, 1100, height=650,position=center');"title="Click to add book" >Add Periodical</a></li>
                           		</li>
							<!-- ENDS init sidebar -->
						</ul>
						<!-- ENDS sidebar -->
