...
?>		
         ...
         <?php if($developer || $super_admin || $librarian){?>
               <li class="active"><a href="logs.php" ><i class="shortcut-icon icon-calendar"></i><span class="shortcut-label">Library Logs</span></a></li>
        <?php } ?>
        <?php if($developer || $super_admin || $librarian){?>
...
<!-- MAIN -->
			<div id="main">
				...
				<div class="wrapper">
					...
					<div id="content">
						...
						<!-- page-content -->
						<div id="page-content">
        <form method="post" id="contactForm validation-form" action="logs.php" >
		<br>
        <div id="result"></div> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
        &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; 	     
         <input class="input-large" name="account_no" type="text" style=" width:auto" placeholder="Library ID"title="Enter Library ID">
		     <input type="submit" value="ADD" name="add" id="submit"  title="click button to add"/>
		</form>
        ...
        <?php
				...
				{
				require "db.php";	
				$e_id = $_POST['account_no'];
			 	$log_in_time =gmdate(" H:i:s",time()+28800);
				$log_date = date('y-m-d H:m:s');
				mysql_query("start transaction");
				mysql_query("Auto-Commit='off'");

				
					$res3 = mysql_query("select *, count(account_no) as count1 from employee_account_t where emp_id = '$e_id'");
					while ($r3 = mysql_fetch_array($res3))
					{
					$account_no=$r3['account_no'];
					$employee = $r3['count1'];
					}
					$res4 = mysql_query("select *, count(account_no) as count2 from student_account_t where student_id = '$e_id'");
					while ($r4 = mysql_fetch_array($res4))
					{
					$account_no1=$r4['account_no'];
					$student= $r4['count2'];
					}
				$res = mysql_query("select count(account_no) as count from library_logs where account_no = '$account_no1' and log_out_time = 0 ");
				while ($r = mysql_fetch_array($res))
					{
					$r1 = $r['count'];
					}
					
				$res1 = mysql_query("select count(account_no) as count from library_logs where account_no = '$account_no' and log_out_time = 0 ");
				...
				if($e_id!=NULL){
					if ($r1 == 0 && $r5 == 0){
						if($student==1){		
						mysql_query("INSERT INTO library_logs values('','$account_no1','$log_in_time','','$log_date')");
						}
						else if( $employee==1){		
						mysql_query("INSERT INTO library_logs values('','$account_no','$log_in_time','','$log_date')");
						}
						else{
						echo"<p class='error-box'>Unregistered ID.</p>";
						}				
					}
				else{
				echo"<p class='error-box'>Library ID is currently log in.</p>";
				}	
				}
		else{
		echo"<p class='error-box'>No input.</p>";
							}	mysql_query("Commit");
				}
  ...
  <div class="widget-content">
   ...
   <table cellpadding="0" cellspacing="0" border="0" class=" " id="example" width="100%" style="">
		...
		<tr>
           ...
           <?php
				require "db.php";	
				$qry = mysql_query("select * from library_logs where log_out_time='0'");
				while ($row = mysql_fetch_array($qry)) {
				$a_no=$row['account_no'];
				$time=$row[log_in_time];
				$time1=date('h:i a', strtotime($time));
				$query1 = mysql_fetch_array(mysql_query("select *, count(account_no) as count2 from student_account_t where account_no = '$a_no'"));
				$count=$query1['count2'];
				$student=$query1['student_id'];
				$query4 = mysql_fetch_array(mysql_query("select * from employee_account_t where account_no = '$a_no'"));
				$employee=$query4['emp_id'];
				if($count==1){
				$query2 = mysql_fetch_array(mysql_query("select * from student_t where student_id = '$student'"));
				echo"<tr align=center>
				<td> <div align='center'>$query2[student_id]</div></td>";?>
				<td><div align='center'>
				<a   class="submit" onClick="window.open('account2.php?call_no=<?php 
							echo "$query2[student_id]";?>&&herf=<?php echo"$herf"; ?>','','toolbar=no, location=no, directories=no, status=no, menubar=no, scrollbars=no, resizable=no, copyhistory=no, width=1100, height=900,position=center');"><?php echo "$query2[f_name]&nbsp$query2[m_name]&nbsp;$query2[l_name]"; ?> </a></div></td>
	     		<?php }
				else
				{
				$query3 = mysql_fetch_array(mysql_query("select * from employee_t where emp_id = '$employee'"));
				echo"<tr align=center>
				<td> <div align='center'>$query3[emp_id]</div></td>";?>
				<td><div align='center'>
				<a   class="submit" onClick="window.open('account2.php?call_no=<?php 
							echo "$employee";?>&&herf=<?php echo"$herf"; ?>','','toolbar=no, location=no, directories=no, status=no, menubar=no, scrollbars=no, resizable=no, copyhistory=no, width=1100, height=900,position=center');"><?php echo "$query3[f_name]&nbsp$query3[m_name]&nbsp;$query3[l_name]"; ?> </a></div></td>
	     		<?php }
				 echo"<td> <div align='center'>$time1 </div></td>
				<td><div align='center'><a href='logsout.php?account_no=$row[account_no]'><img src='img/mono-icons/stop32.png'  alt='stop32.png' class='alignleft'/></a><div align='center'></td>";
...
<script src="../js/plugins/validate/validation.js"></script>
