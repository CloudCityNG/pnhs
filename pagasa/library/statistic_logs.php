...
<!-- title -->
          <div id="result"></div>
						<div id="page-title">
							...
							<span class="subtitle"><br>"A book is like a garden carried in the pocket."<br>  ~Chinese Proverb 
</span>
						</div>
						<!-- ENDS title -->	
  			...
  			<div class="span4">
  				...
  				<div class="widget stacked widget-table">
					...
					<?php 
							...
							FROM library_logs
							JOIN student_account_t ON library_logs.account_no = student_account_t.account_no
							WHERE log_date >='$start1-06-01' and log_date <='$end1-03-30'
				...
				?>
							...
							$query=mysql_query("SELECT *, count(*) as total FROM library_logs  JOIN student_account_t ON 
							library_logs.account_no = student_account_t.account_no GROUP BY student_id ORDER BY total desc limit 10");
							while($c=mysql_fetch_array($query))
							{
							$student_id=$c['student_id'];
							$account_no=$c['account_no'];
							...
							$query1=mysql_query("SELECT *, count(*) FROM library_logs  JOIN employee_account_t ON 
							library_logs.account_no = employee_account_t.account_no GROUP BY emp_id desc limit 10");
...
<a class="btn btn-mini" target="_blank" href="print_stat1.php?year=<?php echo"$year";?>&&level=<?php echo"$level";?>&&section=<?php echo"$section";?>">
...
$(function () {
		...
		var i=0;
			...
			<?php 
							...
							FROM library_logs
							JOIN student_account_t ON library_logs.account_no = student_account_t.account_no
							WHERE log_date >='$start1-06-01' and log_date <='
							$end1-03-30'
							GROUP BY student_id ORDER BY total");
							while($c=mysql_fetch_array($query))
							{
							...
							$query=mysql_query("SELECT *, count(*)as total FROM library_logs  JOIN student_account_t ON 
							library_logs.account_no = student_account_t.account_no GROUP BY student_id desc limit 10");
							while($c=mysql_fetch_array($query))
							{
							$student_id=$c['student_id'];
							$total=$c['total'];
							...
							$query1=mysql_query("SELECT *, count(*) as total FROM library_logs  JOIN employee_account_t ON 
							library_logs.account_no = employee_account_t.account_no GROUP BY emp_id ORDER BY total desc limit 10");
							while($c1=mysql_fetch_array($query1))
							{
							$emp_id=$c1['emp_id'];
							$total=$c1['total'];
