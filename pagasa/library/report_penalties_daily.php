...
<!-- InstanceBeginEditable name="content" -->
    <div id="main">
        ...
        <div id="demo" style="min-height:400px;">
			...
			<thead align="center">
             <tr>
		     <th width="90">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Title</div></th>
             <th  width="90" align="center">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Name</div></th>
             <th  width="70" align="center">Borrowed </th>
             <th  width="70" align="center">Returned </th>
             <th  width="60"align="center">Penalty</th>
             <th width="110">Person In Charge</div></th>
             <th width="55">Remark</div></th>
             </tr>              	
			</thead>
             <tbody>
			...
			$book=mysql_query("SELECT * FROM `appraisal_t` WHERE  return_date >= '$from' AND return_date <= '$to' AND penalties !='0'");
		...
		}$sum=mysql_fetch_array(mysql_query("SELECT sum(penalties) as sum FROM `appraisal_t`WHERE return_date >= '$from' AND return_date <= '$to' AND penalties != '0' AND a_remark='paid'"));
			$sum1=$sum['sum'];
...
?><tr>
		<?php if($book1 == NULL){ ?>
			...
			<td><div align='center'>
				  <a   class="submit" onClick="window.open('account2.php?call_no=<?php 
							echo "$call";?>&&herf=<?php echo"$herf"; ?>','','toolbar=no, location=no, directories=no, status=no, menubar=no, scrollbars=no, resizable=no, copyhistory=no, width=1100, height=900,position=center');"><?php echo "$fname $mname $lname"; ?> </a></div></td>
	        <?php
			 echo"
			<td><div align='center'>$bdate1<br>$btime1</div</td>
			<td><div align='center'>$rdate1<br>$rtime1</div</td>	  
			<td><div align='center'>Php. $penaltiess</div</td>	  
			...
			$book=mysql_query("SELECT * FROM `appraisal_t` WHERE  penalties !='0'");
			while($a=mysql_fetch_array($book))
			{	
			$bdate=$a['borrow_date'];
			$btime=$a['borrow_time'];
		...
		}$sum=mysql_fetch_array(mysql_query("SELECT sum(penalties) as sum FROM `appraisal_t`WHERE  penalties !='0' AND a_remark='paid'"));
			$sum1=$sum['sum'];
			$total=number_format($sum1, 2, '.', ',');
			?><tr>
		<?php if($book1 == NULL){ ?>
			...
			<td><div align='center'> $remark1 </div</td>	  
			  ...
			  <td><div align='center'> -------------- </div></td>
               ...
               <br><br>         <?php if($to==NULL){
				?>
                  <a class="btn btn-block" href="print_penalties.php?action	=<?php echo"$action"; ?>" style="background:#CCCCCC"> 
                 <img src="img/mono-icons/printer32.png" title="PRINT" width="40" height="40"><br>Print</a>
	<?php	}else{		

					?>
                  <a class="btn btn-block" href="print_penalties.php?from=<?php echo"$from"; ?>&&to=<?php echo"$to"; ?>&&action=<?php echo"$action"; ?>"
