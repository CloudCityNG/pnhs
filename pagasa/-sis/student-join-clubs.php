	...
	<div class="subnavbar-inner" >
		...
		<div class="container" >
			...
			<!-- InstanceBeginEditable name="navbar" -->
						  <ul class="mainnav">
			    <li class="active"> <a href="student-join-clubs.php" > <i class="shortcut-icon icon-user"></i> <span class="shortcut-label">Join</span> </a> </li>
		      </ul>
		    <!-- InstanceEndEditable -->
	...
	<!-- InstanceBeginEditable name="content" -->
      ...
      <div class="widget stacked">
       ...
       <?php
		...
		$queryaff = mysql_query("SELECT * FROM club_members_t WHERE student_id = '$sid'");
		$getaff = mysql_fetch_assoc($queryaff);
		$aff = $getaff['club_id'];
       ...
       <table cellpadding="0" cellspacing="0" border="0" class=" dynamic styled with-prev-next" id="example" width="100%">
                ...
                <tbody>
                 ...
                 <?php 
                  ...
                  $queryclub  = mysql_query("SELECT * FROM club_t WHERE club_id != '$aff' AND club_status = 'Active'");
					while($row=mysql_fetch_array($queryclub)){
					
					$id=$row['club_id'];

					$checkexist = 0;	
					$queryexist = mysql_query("SELECT * FROM club_application_t ");
					while($getexist = mysql_fetch_assoc($queryexist)){
					$club=$getexist['club_id'];
					$student=$getexist['student_id'];
					$syear=$getexist['school_year'];
						if($club == $id && $student == $sid && $syear == $current){
						...
						if($checkexist == 0){
							...
							<tr class="del<?php echo $id ?>">
							  ...
							  <td align="center">
							  <a class="btn btn-success" href="sis-actions/join_clubs.php?club_id=<?php echo $id ?>" style="text-decoration:none">Join</a></td>                           
							</tr>
                   <?php
					}
				   } ?> 
