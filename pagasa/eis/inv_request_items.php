  ...
  <div class="container" style="width:1000px; margin-top:20px;">
	...
	<!-- InstanceBeginEditable name="content" -->
      ...
      <?php
	  	error_reporting(0);
		include('../db/db.php');
		$msg = mysql_query("SELECT * FROM supply_message_t, supply_distribution_t, employee_t, account_t, inventory_stock_t, supply_category_t, inventory_item_t, supply_type_t, unit_t, supply_record_t, employee_account_t
							...
							AND supply_distribution_t.sd_id = supply_message_t.sd_id
							...
							AND supply_distribution_t.type =  'ignore'");
				while ($row=mysql_fetch_array($msg)){
									
	 ?>
      <div class="alert alert-danger">
      Your item <?php echo strtoupper($row["description"]." ".$row["category"]." ".$row["item_name"]); ?> can't send beacuse <?php echo $row["message"]; ?>.
      &nbsp;
      <a href="actions/inv_remove_requests.php?message_no=<?php echo $row['message_no'];?>"><i class="icon-remove"></i> </a>
     </div>
      <p>
      <?php } ?>
            ...
            <table cellpadding="0" cellspacing="0" border="0" class=" " id="example" width="100%">
                ...
                <?php 
					...
					/* $query = mysql_query("SELECT *
										...
										AND supply_distribution_t.dept_id = '$dept'
										AND inventory_stock_t.stock_no = supply_record_t.stock_no
										AND supply_record_t.record_id = supply_distribution_t.record_id
										AND department_t.dept_id = employee_t.dept_id");
										
					*/
					
					$query = mysql_query("SELECT * FROM supply_distribution_t, employee_t, account_t, inventory_stock_t, supply_category_t, inventory_item_t, supply_type_t, unit_t, supply_record_t, employee_account_t
							WHERE account_t.username =  '$username'
							AND account_t.account_no = employee_account_t.account_no
                            AND employee_t.emp_id = employee_account_t.emp_id
							AND employee_t.emp_id = supply_distribution_t.emp_id
							...
							AND supply_category_t.item_no = inventory_item_t.item_no
							AND inventory_stock_t.type_no = supply_type_t.type_no
							AND supply_distribution_t.type =  'requested'");					
                        ...
                        <tr class="del<?php echo $id ?>">
                            <td align="center" bgcolor="#ECEEFF" width="90"><?php echo ucfirst($row['date_recieved']); ?></td>
                            <td align="center" bgcolor="#ECEEFF"><?php echo ucfirst($row['description'])." ".ucfirst($row['category'])." ".ucfirst($row['item_name']); ?></td>
                   		    <td align="center" bgcolor="#ECEEFF"><?php $row1=mysql_fetch_array($quantity); echo $row1['quantity']; ?></td>
                            <td align="center" width="100"><?php echo $row['unit_type']; ?></td>
                            
                            <td bgcolor="#ECEEFF" width="100" align="center">
								<a title="CANCEL THE REQUEST" class="btn-mini btn-danger" href="JavaScript:if(confirm('Are you sure you want to delete this record?')==true)
							{window.location='actions/inv_delete_request_action.php?sd_id=<?php echo $row["sd_id"];?>';}"><i class="icon-remove"></i> CANCEL</a>
                            </td>
                        </tr>
                  <?php }  ?>
                </tbody>
