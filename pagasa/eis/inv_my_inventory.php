    ...
    <?php
	  error_reporting(0);
		...
		$sql = mysql_query("SELECT * FROM employee_t, account_t, employee_account_t
					...
					$employee = $row['emp_id']; 
					//echo "".ucfirst($row['f_name'])." ".ucfirst($row['m_name'])." ".ucfirst($row['l_name']).""; 
		
	?>
                <p><h4 align="center">&nbsp;AVAILABLE SUPPLIES IN THE INVENTORY</h4></p>
                ...
                <tbody>
                 ...
                 <?php 
					...
					$query=mysql_query("SELECT *
										FROM inventory_stock_t, supply_category_t, inventory_item_t, supply_type_t, unit_t
                        ...
                        <tr class="del<?php echo $id ?>">
                            <td align="center" width="80"><div align="center"><?php echo $row['stock_no']; ?></div></td>
                            <td><?php echo ucfirst($row['description']); ?></td>
                            <td><?php echo ucfirst($row['category']); ?></td>
                            <td><?php echo ucfirst($row['item_name']); ?></td>
                      		<td align="center"><div align="center"><?php echo ucfirst($row['qoh']); ?></div></td>
                            ...
                            <td align="center" width="80">      
                                ...
                                <a class="btn-mini btn-success" onClick="window.open('inv_get_item.php?stock_no=<?php echo $row["stock_no"];?>','','toolbar=no, location=no, directories=no, status=no, menubar=no, scrollbars=no, resizable=no, copyhistory=no, width=1100, height=600,position=center');"><i class="icon-shopping-cart"></i> GET ITEM</a>
                            </td>
                        </tr>
                     <?php }?>
                </tbody>
        ...
        <p>&nbsp; </p>
        <p>&nbsp; </p>
        <h5>&nbsp;<i class="icon-globe"></i> NOTIFICATION</h5>
      ...
      <?php
		...
		$msg = mysql_query("SELECT * FROM supply_message_t, supply_distribution_t, employee_t, account_t, inventory_stock_t, supply_category_t, inventory_item_t, supply_type_t, unit_t, supply_record_t, employee_account_t
				...
				$row = mysql_num_rows($msg);
									
	 ?>
     <div class="alert alert-danger">
      <?php echo $row ?> items can't be send. <a href="inv_request_items.php"> <br> View Details...</a>
     </div>
      <p>
