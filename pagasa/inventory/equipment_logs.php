            ...
            <table cellpadding="0" cellspacing="0" border="0" class=" " id="example" width="100%">
                ...
                <tbody>
                 ...
                 <?php 
					...
					include('../db/db.php');
					$qry = mysql_query("SELECT * FROM equipment_property_t WHERE status='borrowed'");
				    while($row = mysql_fetch_array($qry)){
					 ...
					 $query =mysql_fetch_array( mysql_query("SELECT * FROM equipment_item_t, equipment_color_t WHERE equipment_item_t.stock_no='$stock_no' 
						...
						?>
                            <td  align="center" width="120"><div align="center" class="style3"><?php echo $row["property_no"]; ?></div></td>
							<td align="center">
                            ...
                            <a class="btn btn-success" onClick="window.open('equipment_mr_print.php?property_no=<?php echo $row['property_no']; ?>','','toolbar=no, location=no, directories=no, status=no, menubar=no, scrollbars=no, resizable=no, copyhistory=no, width=1100, height=600,position=center');"><i class="icon-print"></i>&nbsp;Print</a> 
							<a class="btn btn-info" href="JavaScript:if(confirm('Return the equipment?')==true)
							{window.location='actions/equip_returnMR_action.php?property_no=<?php echo $row['property_no'];?>&stock_no=<?php echo $row['stock_no']; ?>&quantity_acquired=<?php echo $row['quantity_acquired'] ?>'};"><i class="icon-circle-arrow-left"></i>&nbsp;Returned</a>                            </td>
