...
<?php
	if(empty($_GET['participation_num'])){
	?>
	<div id="myModal2" >
  <div class="modal-header">
    <h3>Choose Participation Number</h3>
  </div>
  ...
  <?php
	...
	$quiznn=mysql_query("SELECT * FROM grading_participation WHERE student_id='$stud_id' ORDER BY participation_num DESC LIMIT 1");
			$fetchh = mysql_fetch_assoc($quiznn);
			$participation_num = $fetchh['participation_num'];
			$x = $fetchh['participation_num']; 
	//for ($x; $x<=10; $x++)
	for( $x; $x >= 1; $x-- ) {
...
?>
    ...
    <p>
 <a class="btn btn-small btn-primary"  data-dismiss="modal" onClick="window.open('grading_participation_grade.php?section_id=<?php echo $sec_id=$_GET['section_id'];?>&participation_num=<?php echo $x; ?>','','toolbar=no, location=no, directories=no, status=no, menubar=no, scrollbars=no, resizable=no, copyhistory=no, width=850, height=900,position=center'); ">#<?php echo $x; ?> Participation</a><br><br>
...
<table class="table table-striped table-bordered" id="example">
						...
						<th class="accordion">Participation</th>
             ...
             <?php 
					include('../db/db.php');
						...
						$participation_num = $_GET['participation_num'];		
						$query_reg1=mysql_query("SELECT * FROM grading_participation WHERE student_id='$id' and participation_num='$participation_num'");
						while($fetch=mysql_fetch_assoc($query_reg1)){
							$participation_num = $fetch['participation_num'];
							$participation_grade = $fetch['participation_grade'];
							?>
					    ...
					    <tr class="del<?php echo $id ?>">
							...
							<td><?php echo $participation_num; ?></td>
							  <td><?php echo $participation_grade; ?></td>
                        </tr>
