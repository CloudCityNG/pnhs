...
<?php 
 ...
 <?php
	if(empty($_GET['proj_num'])){
	?>
	<div id="myModal2" >
  <div class="modal-header">
    <h3>Choose Project Number</h3>
  </div>
  
  <div class="modal-body">
  <?php
	$query_reg12=mysql_query("SELECT * FROM student_registration_t WHERE school_yr=$sch_yr");
						while($row_reg=mysql_fetch_assoc($query_reg12)){
							$stud_id=$row_reg['student_id'];	
								}
	$quiznn=mysql_query("SELECT * FROM grading_projects WHERE student_id='$stud_id' ORDER BY proj_num DESC LIMIT 1");
			$fetchh = mysql_fetch_assoc($quiznn);
			$proj_num = $fetchh['proj_num'];
			$x = $fetchh['proj_num']; 
	//for ($x; $x<=10; $x++)
	for( $x; $x >= 1; $x-- ) {
...
?>
    ...
    <p>
 <a class="btn btn-small btn-primary"  data-dismiss="modal" onClick="window.open('grading_project_grade.php?section_id=<?php echo $sec_id=$_GET['section_id'];?>&proj_num=<?php echo $x; ?>','','toolbar=no, location=no, directories=no, status=no, menubar=no, scrollbars=no, resizable=no, copyhistory=no, width=850, height=900,position=center'); ">#<?php echo $x; ?> Project</a><br><br>
...
<table class="table table-striped table-bordered" id="example">
             ...
             <?php 
					include('../db/db.php');
						...
						$proj_num = $_GET['proj_num'];
						$query_reg1=mysql_query("SELECT * FROM grading_projects WHERE student_id='$id' and proj_num='$proj_num'");
						while($fetch=mysql_fetch_assoc($query_reg1)){
							$proj_num = $fetch['proj_num'];
							$proj_grade = $fetch['proj_grade'];
							?>
					    ...
					    <tr class="del<?php echo $id ?>">
							...
							<td><?php echo $proj_num; ?></td>
							  <td><?php echo $proj_grade; ?></td>
                        </tr>
