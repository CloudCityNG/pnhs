...
<?php
	if(empty($_GET['chapter_test_num'])){
	?>
	<div id="myModal2" >
  <div class="modal-header">
    <h3>Choose Long Exam Number</h3>
  </div>
  ...
  <?php
	...
	$quiznn=mysql_query("SELECT * FROM grading_chapter_test WHERE student_id='$stud_id' ORDER BY chapter_test_num DESC LIMIT 1");
			$fetchh = mysql_fetch_assoc($quiznn);
			$chapter_test_num = $fetchh['chapter_test_num'];
			$x = $fetchh['chapter_test_num']; 
	//for ($x; $x<=10; $x++)
	for( $x; $x >= 1; $x-- ) {
...
?>
    ...
    <p>
 <a class="btn btn-small btn-primary"  data-dismiss="modal" onClick="window.open('grading_longexam_grade.php?section_id=<?php echo $sec_id=$_GET['section_id'];?>&chapter_test_num=<?php echo $x; ?>','','toolbar=no, location=no, directories=no, status=no, menubar=no, scrollbars=no, resizable=no, copyhistory=no, width=850, height=900,position=center'); ">#<?php echo $x; ?> Long Exam</a><br><br>
...
<table class="table table-striped table-bordered" id="example">
						...
						<th class="accordion">Long Exam #</th>
             ...
             <?php 
					include('../db/db.php');
						...
						$chapter_test_num = $_GET['chapter_test_num'];		
						$query_reg1=mysql_query("SELECT * FROM grading_chapter_test WHERE student_id='$id' and chapter_test_num='$chapter_test_num'");
						while($fetch=mysql_fetch_assoc($query_reg1)){
							$chapter_test_num = $fetch['chapter_test_num'];
							$chapter_test_score = $fetch['chapter_test_score'];
							$chapter_test_item_num = $fetch['chapter_test_item_num'];
							$chapter_test_avg = $fetch['chapter_test_avg'];
							$chapter_test_date = $fetch['chapter_test_date'];
							?>
					    ...
					    <tr class="del<?php echo $id ?>">
							...
							<td><?php echo $chapter_test_num; ?></td>
                             <td><?php echo $chapter_test_score; ?></td>
							  <td><?php echo $chapter_test_item_num; ?></td>
							  <td><?php echo $chapter_test_avg; ?></td>
							   <td><?php echo $chapter_test_date; ?></td>
