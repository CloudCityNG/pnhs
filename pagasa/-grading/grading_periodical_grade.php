...
<?php 
  ...
  <?php
	if(empty($_GET['periodical_exam_num'])){
	?>
	<div id="myModal2" >
  <div class="modal-header">
    <h3>Choose Periodical Exam Number</h3>
  </div>
  ...
  <?php
	...
	$quiznn=mysql_query("SELECT * FROM grading_periodical_exam WHERE student_id='$stud_id' ORDER BY periodical_exam_num DESC LIMIT 1");
			$fetchh = mysql_fetch_assoc($quiznn);
			$periodical_exam_num = $fetchh['periodical_exam_num'];
			$x = $fetchh['periodical_exam_num']; 
	//for ($x; $x<=10; $x++)
	for( $x; $x >= 1; $x-- ) {
...
?>
    ...
    <p>
 <a class="btn btn-small btn-primary"  data-dismiss="modal" onClick="window.open('grading_periodical_grade.php?section_id=<?php echo $sec_id=$_GET['section_id'];?>&periodical_exam_num=<?php echo $x; ?>','','toolbar=no, location=no, directories=no, status=no, menubar=no, scrollbars=no, resizable=no, copyhistory=no, width=850, height=900,position=center'); ">#<?php echo $x; ?> Periodical Exam</a><br><br>
...
<table class="table table-striped table-bordered" id="example">
						...
						<th class="accordion">Periodical Exam #</th>
             ...
             <?php 
					include('../db/db.php');
						...
						$periodical_exam_num = $_GET['periodical_exam_num'];		
						$query_reg1=mysql_query("SELECT * FROM grading_periodical_exam WHERE student_id='$id' and periodical_exam_num='$periodical_exam_num'");
						while($fetch=mysql_fetch_assoc($query_reg1)){
							$periodical_exam_num = $fetch['periodical_exam_num'];
							$periodical_exam_score = $fetch['periodical_exam_score'];
							$periodical_exam_item_num = $fetch['periodical_exam_item_num'];
							$periodical_exam_avg = $fetch['periodical_exam_avg'];
							$periodical_exam_date = $fetch['periodical_exam_date'];
							?>
					    ...
					    <tr class="del<?php echo $id ?>">
							...
							<td><?php echo $periodical_exam_num; ?></td>
                             <td><?php echo $periodical_exam_score; ?></td>
							  <td><?php echo $periodical_exam_item_num; ?></td>
							  <td><?php echo $periodical_exam_avg; ?></td>
							   <td><?php echo $periodical_exam_date; ?></td>
