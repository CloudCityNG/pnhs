<?php
include('../../db/db.php');
	$assign_item_num = $_POST['assignItem'];
	$assign_date = $_POST['Date'];
	$grading_period = $_POST['RadioGroup1'];
	$b = $_POST['subject_code'];

	if(empty($assign_item_num) || empty($assign_date) || empty($grading_period)){
...
<form><p>
                            ...
                            <?php echo $b; ?>
           					...
           					No. of Items: <?php echo $assign_item_num; ?>  </label>
                            <br><br>
                           <label>Date: <?php echo $assign_date; ?>
					...
					foreach ($_POST['grade'] as $key => $value) {
					    ...
					    <tr class="del<?php echo $student_id ?>">
                            ...
                            <td><?php echo strtoupper("".$row_stud['l_name']." , ".$row_stud['f_name'].""); ?>
                             ...
                             <td><?php echo $assign_score;
							
							//if($count > 0){
							$assign=mysql_query("SELECT * FROM grading_assignments WHERE student_id='$student_id' ORDER BY assign_num DESC LIMIT 1");
							$fetchh = mysql_fetch_assoc($assign);
							$assign_num = $fetchh['assign_num'] + 1;
							$qd = $_POST['Date'];
							$assign_avg = $assign_score / $assign_item_num * 100;
							mysql_query("INSERT INTO grading_assignments (student_id, assign_num, assign_item_num, assign_score, assign_avg, assign_date, grading_period, subject_code) 
							VALUES ('$student_id', '$assign_num', '$assign_item_num', '$assign_score', '$assign_avg', '$qd', '$grading_period', '$b')") or die("Error in data insertion.".mysql_error());
							$select = mysql_query("SELECT assignments FROM grading_criteria");
							$fetch = mysql_fetch_array($select);
							$qui = ".".$fetch['assignments'];
							$q_avg= $assign_avg * $qui;							
							//$sql = mysql_query("INSERT INTO grading_subject_grade_assignment (assign_id, assign_avg) 
							//VALUES (LAST_INSERT_ID(),'$q_avg')") or die("Error in data insertion.".mysql_error());
							
							 ?></td>
                        </tr>
						<?php }
						else if( $assign_score > $assign_item_num ){
							echo "<script>alert('You entered a score higher than the assign item')</script><body onload='history.go(-1);'>"; 
								}else {
