<?php
include('../../db/db.php');
	$chapter_test_item_num = $_POST['longexamItem'];
	$chapter_test_date = $_POST['Date'];
	$grading_period = $_POST['RadioGroup1'];
	$b=$_POST['subject_code'];

	if(empty($chapter_test_item_num) || empty($chapter_test_date) || empty($grading_period)){
...
<form><p>
                 			...
                 			<br> <br>
           					...
           					No. of Items: <?php echo $chapter_test_item_num; ?>  </label>
                            <br><br>
                           <label>Date: <?php echo $chapter_test_date; ?>
					...
					foreach ($_POST['grade'] as $key => $value) {
					    ...
					    <tr class="del<?php echo $student_id ?>">
                            ...
                            <td><?php echo strtoupper("".$row_stud['l_name']." , ".$row_stud['f_name'].""); ?>
                             ...
                             <td><?php echo $chapter_test_score;
							$longexamm=mysql_query("SELECT * FROM grading_chapter_test WHERE student_id='$student_id' ORDER BY chapter_test_num DESC LIMIT 1");
							$fetchh = mysql_fetch_assoc($longexamm);
							$chapter_test_num = $fetchh['chapter_test_num'] + 1;
							
							$qd = $_POST['Date'];
							$chapter_test_avg = $chapter_test_score / $chapter_test_item_num * 100;
							mysql_query("INSERT INTO grading_chapter_test (student_id, chapter_test_num, chapter_test_item_num, chapter_test_score, chapter_test_avg, chapter_test_date, grading_period, subject_code) 
							VALUES ('$student_id', '$chapter_test_num', '$chapter_test_item_num', '$chapter_test_score', '$chapter_test_avg', '$qd', $grading_period, $b)") or die("Error in data insertion.".mysql_error());
							$select = mysql_query("SELECT chapter_test FROM grading_criteria");
							$fetch = mysql_fetch_array($select);
							$qui = ".".$fetch['chapter_test'];
							$q_avg= $chapter_test_avg * $qui;
							$sql = mysql_query("INSERT INTO grading_subject_grade_chapter_test (chapter_test_id, chapter_test_avg) 
