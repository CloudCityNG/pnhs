...
<table class="table table-striped table-bordered" id="example">
                		...
                		<th class="accordion">Grade</th>
             <?php 
					foreach ($_POST['grade'] as $key => $value) {
								
								$student_id = $_POST['student_id'][$key];
								$participation_grade = $_POST['grade'][$key];
								
							//if($participation_grade < 0){
							
							//}else {
								$sql = mysql_query("SELECT * FROM grading_participation WHERE student_id=$student_id");
					    ...
					    <tr class="del<?php echo $student_id ?>">
                            ...
                            <td><?php echo strtoupper("".$row_stud['l_name']." , ".$row_stud['f_name'].""); ?>
                             ...
                             <td><?php echo $participation_grade;
							$parti=mysql_query("SELECT * FROM grading_participation WHERE student_id='$student_id' ORDER BY participation_num DESC LIMIT 1");
							$fetchh = mysql_fetch_assoc($parti);
							$participation_num = $fetchh['participation_num'] + 1;
							
							mysql_query("INSERT INTO grading_participation (student_id, participation_num, participation_grade, grading_period, subject_code) 
							VALUES ('$student_id', '$participation_num', '$participation_grade', '$grading_period', $b)") or die("Error in data insertion.".mysql_error());
							$select = mysql_query("SELECT participation FROM grading_criteria");
							$fetch = mysql_fetch_array($select);
							$qui = ".".$fetch['participation'];
							$q_grade= $participation_grade * $qui;
							$sql = mysql_query("INSERT INTO grading_subject_grade_participation (participation_id, participation_avg) 
							VALUES (LAST_INSERT_ID(),'$q_grade')") or die("Error in data insertion.".mysql_error());
							 ?></td>
                        </tr>
						<?php } else {
