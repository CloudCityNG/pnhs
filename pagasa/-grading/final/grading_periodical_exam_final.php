<?php
include('../../db/db.php');
	$periodical_exam_item_num = $_POST['periodical_examItem'];
	$periodical_exam_date = $_POST['Date'];
	$grading_period = $_POST['RadioGroup1'];

	if(empty($periodical_exam_item_num) || empty($periodical_exam_date)/* || empty($grading_period)*/){
...
<form><p>
                 			...
                 			<br> <br>
           					...
           					No. of Items: <?php echo $periodical_exam_item_num; ?>  </label>
                            <br><br>
                           <label>Date: <?php echo $periodical_exam_date; ?>
...
<table class="table table-striped table-bordered" id="example">
             ...
             <?php 
					foreach ($_POST['grade'] as $key => $value) {
								...
								$periodical_exam_score = $_POST['grade'][$key];
								
							//if($periodical_exam_score < 0){
							
							//}else {
								$sql = mysql_query("SELECT * FROM grading_periodical_exam WHERE student_id=$student_id");
					    ...
					    <tr class="del<?php echo $student_id ?>">
                            ...
                            <td><?php echo strtoupper("".$row_stud['l_name']." , ".$row_stud['f_name'].""); ?>
                             ...
                             <td><?php echo $periodical_exam_score;
							$periodical=mysql_query("SELECT * FROM grading_periodical_exam WHERE student_id='$student_id' ORDER BY periodical_exam_num DESC LIMIT 1");
							$fetchh = mysql_fetch_assoc($periodical);
							$periodical_exam_num = $fetchh['periodical_exam_num'] + 1;
							
							$qd = $_POST['Date'];
							$periodical_exam_avg = $periodical_exam_score / $periodical_exam_item_num * 100;
							mysql_query("INSERT INTO grading_periodical_exam (student_id, periodical_exam_num, periodical_exam_item_num, periodical_exam_score, periodical_exam_avg, periodical_exam_date) 
							VALUES ('$student_id', '$periodical_exam_num', '$periodical_exam_item_num', '$periodical_exam_score', '$periodical_exam_avg', '$qd')") or die("Error in data insertion.".mysql_error());
							$select = mysql_query("SELECT periodical_exam FROM grading_criteria");
							$fetch = mysql_fetch_array($select);
							$qui = ".".$fetch['periodical_exam'];
							$q_avg= $periodical_exam_avg * $qui;
							$sql = mysql_query("INSERT INTO grading_subject_grade_periodical_exam (periodical_exam_id, periodical_exam_avg) 
