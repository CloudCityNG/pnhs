...
<table class="table table-striped table-bordered" id="example">
             ...
             <?php 
					foreach ($_POST['grade'] as $key => $value) {
								...
								$proj_grade = $_POST['grade'][$key];
								
							//if($proj_grade < 0){
							
							//}else {
								$sql = mysql_query("SELECT * FROM grading_projects WHERE student_id=$student_id");
					    ...
					    <tr class="del<?php echo $student_id ?>">
                            ...
                            <td><?php echo strtoupper("".$row_stud['l_name']." , ".$row_stud['f_name'].""); ?>
                             ...
                             <td><?php echo $proj_grade;
							$project=mysql_query("SELECT * FROM grading_projects WHERE student_id='$student_id' ORDER BY proj_num DESC LIMIT 1");
							$fetchh = mysql_fetch_assoc($project);
							$proj_num = $fetchh['proj_num'] + 1; 
							 
							mysql_query("INSERT INTO grading_projects (student_id, proj_num, proj_grade, grading_period, subject_code) 
							VALUES ('$student_id', '$proj_num', '$proj_grade', '$grading_period', '$b')") or die("Error in data insertion.".mysql_error());
							$select = mysql_query("SELECT projects FROM grading_criteria");
							$fetch = mysql_fetch_array($select);
							$qui = ".".$fetch['projects'];
							$q_grade= $proj_grade * $qui;
							$sql = mysql_query("INSERT INTO grading_subject_grade_projects (proj_id, proj_avg) 
