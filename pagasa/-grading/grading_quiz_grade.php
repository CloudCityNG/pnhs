...
<?php
  ...
  <?php
	...
	$quiznn=mysql_query("SELECT * FROM grading_quizzes WHERE student_id='$stud_id' ORDER BY quiz_num DESC LIMIT 1");
			$fetchh = mysql_fetch_assoc($quiznn);
			$quiz_num = $fetchh['quiz_num'];
			$x = $fetchh['quiz_num']; 
	//for ($x; $x<=10; $x++)
	for( $x; $x >= 1; $x-- ) {
...
?>
    ...
    <p>
 <a class="btn btn-small btn-primary"  data-dismiss="modal" onClick="window.open('grading_quiz_grade.php?section_id=<?php echo $sec_id=$_GET['section_id'];?>&quiz_number=<?php echo $x; ?>','','toolbar=no, location=no, directories=no, status=no, menubar=no, scrollbars=no, resizable=no, copyhistory=no, width=850, height=900,position=center'); ">#<?php echo $x; ?> Quiz</a><br><br>
...
<table class="table table-striped table-bordered" id="example">
						Quiz #<?php echo $_GET['quiz_number'];?>
                 		<th class="accordion">
                 		  Student_ID</th>
                        <th class="accordion">Name</th>
						<th class="accordion">Quiz #</th>
             ...
             <?php 
					include('../db/db.php');
						...
						$query_reg1=mysql_query("SELECT * FROM grading_quizzes WHERE student_id='$id' and quiz_num='$quiz_number'");
						while($fetch=mysql_fetch_assoc($query_reg1)){
							$quiz_num = $fetch['quiz_num'];
							$quiz_score = $fetch['quiz_score'];
							$quiz_item_num = $fetch['quiz_item_num'];
							$quiz_avg = $fetch['quiz_avg'];
							$quiz_date = $fetch['quiz_date'];
							?>
					    ...
					    <tr class="del<?php echo $id ?>">
							...
							<td><?php echo $quiz_num; ?></td>
                             <td><?php echo $quiz_score; ?></td>
							  <td><?php echo $quiz_item_num; ?></td>
							  <td><?php echo $quiz_avg; ?></td>
