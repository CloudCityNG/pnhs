    ...
    <div class="widget stacked widget-table action-table">
        ...
        <div class="widget-content" >
          <table class="table table-striped table-bordered">
            <thead>
              <tr>
                <th width="80%"><i class="icon-ambulance"></i> Teachers</th>
                <th></th>
              </tr>
            </thead>
            <tbody>
           ...
           <div id="demo" style="min-height:400px;">
            <table cellpadding="0" cellspacing="0" border="0" class=" " id="example" width="100%">
                ...
                <tbody>
                 ...
                 <?php 				     
					include('../db/db.php');
					$query = mysql_query("SELECT * FROM employee_t WHERE emp_id IN (SELECT emp_id FROM teacher_t)");
					
					
					while($row = mysql_fetch_assoc($query)){
					$emp_id=$row['emp_id']; 
					
					$query_teacher = mysql_query("SELECT max_load FROM teacher_t WHERE emp_id={$emp_id}") or die(mysql_error());
					$row_teacher = mysql_fetch_assoc($query_teacher);
					$max_load = $row_teacher['max_load'];
					?>
                    
                    <tr class=" odd del<?php echo $section_id; ?>">
                      <td ><?php echo $emp_id; ?></td>
                      <td><?php echo $row['f_name']." ".$row['l_name']; ?></td>
                      <td><center><?php echo $max_load; ?></center></td>
                      <td><a class='btn btn-mini' href='stat_teacher.php?emp_id=<?php echo $emp_id;?>'> STATISTICS </a></td>
                      <td align="center">
                          <a class='btn btn-mini' href='report_teacher_view.php?emp_id=<?php echo $emp_id;?>'> LOAD </a>
                          <a class='btn btn-mini' href='sched_teacher_view.php?emp_id=<?php echo $emp_id;?>'> SCHEDULE </a>
                      </td>
